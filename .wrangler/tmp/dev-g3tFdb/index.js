...
app.post("/api/blogs", async (c) => {
  const { title: title2, content, image } = await c.req.json();
  if (!title2 || !content) {
    return c.json({
      success: false,
  ...
  try {
    ...
    `).bind(title2, content, image || null).run();
...
// .wrangler/tmp/bundle-yuW2mq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yuW2mq/middleware-loader.entry.ts
